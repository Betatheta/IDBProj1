...
@WebServlet("/DecideResultServlet")
public class DecideResultServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			if(planId==null) {
				pId = sql.isDetermined(eId);
			}
			else pId = Integer.valueOf(planId);
			
			sql.determineAnEvent(eId, pId);
			...
			pw.println("<li class=\"nav-header\">Decision of Event</li>");
			...
			for (int i = 0; i < planList.size(); i++) {
				...
				pw.print("<li><a onmouseover=\"toggleBounce(" + i
						...
						+ "</span>");
				if (pId == p.pid)
					pw.println("<font color=\"8B2500\">"  + time+" @ "+ address
							+ "</font></label></a></li>");
				else
	...
	private void printHeader(PrintWriter pw) {
		...
		pw.println("<title>Plangout | Decision of event</title>");
