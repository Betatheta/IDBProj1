...
public class VoteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (eventId == null || facebookId == null) {
		...
		try {
			...
			pw.println("<form id=\"vote\" name=\"voteform\" action=\"voteresult\">");

			pw.println("<div class=\"well sidebar-nav\">");
			pw.println("<ul class=\"nav nav-list\">");
			pw.println("<li class=\"nav-header\">Choose A Plan</li>");
			...
			for (int i = 0; i < planList.size(); i++) {
				...
				String Lng = location.substring(i_comma + 1, i_colon - 1);
				String address = location.substring(i_colon + 1);
				pw.println("<li><a onmouseover=\"toggleBounce("
						+ i
						+ ")\" onmouseout=\"toggleBounce("
						+ i
						+ ")\"><label><input type=\"radio\" name=\"planId\" onclick=\"onClick()\" value="
						+ p.pid + ">" + time+" @ "+address + "</input></label></a></li>");
			}
			pw.println("</ul>");
			pw.println("</div>");
			pw.println("<input type=\"hidden\" name=\"eventId\" value="
					+ event.eid + " />");
	...
	private void printHeader(PrintWriter pw) {
		...
		pw.println("<title>Plangout | Vote for event</title>");
	...
	private void printRest(PrintWriter pw) {
		...
		pw.println("<button id=\"votebtn\" class=\"btn btn-primary\" disabled=\"disabled\" onclick=\"vote()\">Submit</button>");
		pw.println("</div>");
		pw.println("<script>");
		pw.println("function vote() {");
